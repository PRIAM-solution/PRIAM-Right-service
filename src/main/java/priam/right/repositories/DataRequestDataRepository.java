...
public interface DataRequestDataRepository extends JpaRepository<DataRequestData, Integer> {
    ...
    @Query(value= "SELECT answer_by_data FROM DataRequest_Data WHERE data_id =:dataId AND data_request_id IN " +
            "(SELECT data_request_id FROM DataRequest WHERE data_subject_id = :dataSubjectId AND data_request_issued_at BETWEEN" +
